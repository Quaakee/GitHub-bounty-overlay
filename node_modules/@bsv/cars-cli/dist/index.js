...
const VALID_LOG_PERIODS = ['5m', '15m', '30m', '1h', '2h', '6h', '12h', '1d', '2d', '7d'];
const VALID_LOG_LEVELS = ['all', 'error', 'warn', 'info'];
...
function saveCARSConfigInfo(info) {
    fs.writeFileSync(CONFIG_PATH, JSON.stringify(info, null, 2));
}
function isCARSConfig(c) {
    return c.provider === 'CARS';
}
function listAllConfigs(info) {
    return info.configs || [];
}
function printAllConfigsWithIndex(info) {
...
function findConfigByNameOrIndex(info, nameOrIndex) {
    const all = listAllConfigs(info);
 ...
 */
async function pickCARSConfig(info, nameOrIndex) {
    const all = listAllConfigs(info);
...
async function ensureRegistered(carsConfig) {
 ...
 */
async function addCARSConfigInteractive(info) {
    const cloudChoices = [
...
async function editCARSConfigInteractive(info, config) {
    const cloudChoices = [
...
function deleteCARSConfig(info, config) {
 ...
 * Build logic
 */
async function buildArtifact(nameOrIndex) {
 ...
 * Helper for requests
 */
async function safeRequest(client, baseUrl, endpoint, data) {
    try {
 ...
 * Error handling
 */
function handleRequestError(error, contextMsg) {
    if (contextMsg)
...
function printAdminsList(admins) {
...
function printLogs(log, title) {
    console.log(chalk.blue(`${title}:`));
    console.log(log.trim() || chalk.yellow('No logs yet.'));
}
function printReleasesList(deploys) {
 ...
 */
function getDistinctCARSCloudURLs(info) {
    const urls = (info.configs || [])
        .filter(isCARSConfig)
        .map(c => c.CARSCloudURL)
        .filter(u => !!u);
    return Array.from(new Set(urls));
}
async function chooseCARSCloudURL(info, specifiedNameOrIndex) {
...
async function buildAuthFetch(config) {
 ...
 */
async function pickReleaseId(config, providedReleaseId) {
    if (providedReleaseId) {
        return providedReleaseId;
    }
    const client = await buildAuthFetch(config);
    const result = await safeRequest(client, config.CARSCloudURL, `/api/v1/project/${config.projectID}/deploys/list`, {});
    ...
    const { tail } = await inquirer.prompt([
        {
            ...
            default: 1000,
            validate: (val) => val > 0 && val <= MAX_TAIL_LINES ? true : 'Invalid tail number'
        }
    ]);
...
async function fetchResourceLogs(config, params) {
    if (!config.projectID) {
    ...
    const client = await buildAuthFetch(config);
    const result = await safeRequest(client, config.CARSCloudURL, `/api/v1/project/${config.projectID}/logs/resource/${finalParams.resource}`, { since: finalParams.since, tail: tailVal, level: finalParams.level });
 ...
 */
// Print instructions
function printDomainInstrictions(projectID, domain, domainType) {
...
async function setCustomDomain(config, domainType, domain, interactive) {
    ...
    while (retry) {
        try {
            let result = await client.fetch(`${config.CARSCloudURL}/api/v1/project/${config.projectID}/domains/${domainType}`, {
                method: 'POST',
                headers: {
                    'content-type': 'application/json'
                },
...
async function viewAndEditWebUIConfig(config) {
    ...
    const client = await buildAuthFetch(config);
    // Fetch current info
    const info = await safeRequest(client, config.CARSCloudURL, `/api/v1/project/${config.projectID}/info`, {
        method: 'POST',
        headers: {
            'content-type': 'application/json'
        },
...
async function viewBillingStats(config) {
    ...
    if (type !== 'all')
        data.type = type;
    const records = await safeRequest(client, config.CARSCloudURL, `/api/v1/project/${config.projectID}/billing/stats`, data);
    if (!records)
        return;
 ...
 */
async function showProjectInfo(config) {
    if (!config.projectID) {
        console.error(chalk.red('❌ No project ID set.'));
        return;
    }
...
async function editAdvancedEngineConfig(config) {
    ...
    const infoResp = await safeRequest(client, config.CARSCloudURL, `/api/v1/project/${config.projectID}/info`, {});
    if (!infoResp)
        return;
    let engineConfig = {};
    ...
    while (!done) {
        ...
        const updateResult = await safeRequest(client, config.CARSCloudURL, `/api/v1/project/${config.projectID}/settings/update`, { ...engineConfig } // we flatten them in request
        );
...
async function triggerAdminEndpoint(config, endpoint) {
    if (!config.projectID) {
        console.error(chalk.red('❌ No project ID set.'));
        return;
    }
...
async function projectMenu() {
    ...
    while (!done) {
        ...
        if (action === 'ls') {
            ...
            const client = await buildAuthFetch(config);
            const result = await safeRequest(client, config.CARSCloudURL, `/api/v1/project/${config.projectID}/admins/list`, {});
            if (result) {
            ...
            const client = await buildAuthFetch(config);
            const result = await safeRequest(client, config.CARSCloudURL, `/api/v1/project/${config.projectID}/logs/project`, {});
            if (result && typeof result.logs === 'string') {
...
async function releaseMenu() {
    ...
    while (!done) {
        ...
        if (action === 'get-upload-url') {
            ...
            const client = await buildAuthFetch(config);
            const result = await safeRequest(client, config.CARSCloudURL, `/api/v1/project/${config.projectID}/deploy`, {});
            ...
            const client = await buildAuthFetch(config);
            const result = await safeRequest(client, config.CARSCloudURL, `/api/v1/project/${config.projectID}/logs/deployment/${releaseId}`, {});
            if (result && typeof result.logs === 'string') {
            ...
            const client = await buildAuthFetch(config);
            const result = await safeRequest(client, config.CARSCloudURL, `/api/v1/project/${config.projectID}/logs/deployment/${deploymentId}`, {});
            if (result && typeof result.logs === 'string') {
                printLogs(result.logs, 'Release Logs');
            }
        }
 ...
 *
 */
async function uploadArtifact(uploadURL, artifactPath) {
...
projectCommand
    ...
    const client = await buildAuthFetch(cfg);
    const result = await safeRequest(client, cfg.CARSCloudURL, `/api/v1/project/${cfg.projectID}/admins/list`, {});
    if (result && result.admins)
        printAdminsList(result.admins);
});
// Project logs
projectCommand
    ...
    const client = await buildAuthFetch(cfg);
    const result = await safeRequest(client, cfg.CARSCloudURL, `/api/v1/project/${cfg.projectID}/logs/project`, {});
    if (result)
        printLogs(result.logs, 'Project Logs');
});
// Resource logs
...
projectCommand
    ...
    const client = await buildAuthFetch(cfg);
    const result = await safeRequest(client, cfg.CARSCloudURL, `/api/v1/project/${cfg.projectID}/deploys/list`, {});
    if (result && Array.isArray(result.deploys)) {
        printReleasesList(result.deploys);
    }
});
...
projectCommand
    ...
    const client = await buildAuthFetch(cfg);
    const projectInfo = await safeRequest(client, cfg.CARSCloudURL, `/api/v1/project/${cfg.projectID}/info`, {});
...
projectCommand
    ...
    const data = {};
    if (options.start)
        data.start = new Date(options.start.trim()).toISOString();
    if (options.end)
        data.end = new Date(options.end.trim()).toISOString();
    if (options.type && options.type !== 'all')
        data.type = options.type;
    const client = await buildAuthFetch(cfg);
    const records = await safeRequest(client, cfg.CARSCloudURL, `/api/v1/project/${cfg.projectID}/billing/stats`, data);
    if (!records)
        return;
    if (records.records.length === 0) {
        console.log(chalk.yellow('No billing records found for specified filters.'));
...
releaseCommand
    ...
    const client = await buildAuthFetch(cfg);
    const result = await safeRequest(client, cfg.CARSCloudURL, `/api/v1/project/${cfg.projectID}/deploy`, {});
    if (result && result.url && result.deploymentId) {
        console.log(chalk.green(`✅ Release created. Release ID: ${result.deploymentId}`));
        console.log(`Upload URL: ${result.url}`);
    }
...
releaseCommand
    ...
    if (!finalReleaseId)
        return;
    const client = await buildAuthFetch(cfg);
    const result = await safeRequest(client, cfg.CARSCloudURL, `/api/v1/project/${cfg.projectID}/logs/deployment/${finalReleaseId}`, {});
    if (result)
        printLogs(result.logs, 'Release Logs');
});
