...
class WalletPermissionsManager {
    ...
    async findProtocolToken(originator, privileged, protocolID, counterparty, includeExpired) {
        ...
        for (const out of result.outputs) {
            const script = sdk_1.LockingScript.fromHex(out.lockingScript);
            const dec = sdk_1.PushDrop.decode(script);
    ...
    async findBasketToken(originator, basket, includeExpired) {
        ...
        for (const out of result.outputs) {
            const dec = sdk_1.PushDrop.decode(sdk_1.LockingScript.fromHex(out.lockingScript));
     ...
     */
    async listProtocolPermissions({ originator }) {
        const basketName = BASKET_MAP.protocol;
        const tags = originator ? [`originator ${originator}`] : [];
        const result = await this.underlying.listOutputs({
            basket: basketName,
            tags,
            tagQueryMode: 'all',
