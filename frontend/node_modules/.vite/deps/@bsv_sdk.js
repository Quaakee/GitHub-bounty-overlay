...
var _BigNumber = class _BigNumber {
  ...
  initNumber(number, base, endian) {
    ...
    if (number < 67108864) {
      ...
      this.assert(number <= 9007199254740991, "The number is larger than 2 ^ 53 (unsafe)");
  ...
  static fromBits(bits, strict = false) {
    bits = (bits & 2147483648) !== 0 ? bits - 4294967296 : bits;
...
var SHA512 = class extends BaseHash {
  ...
  _prepareBlock(msg, start) {
    ...
    for (; i < W.length; i += 2) {
      const c0Hi = g1_512_hi(W[i - 4], W[i - 3]);
      const c0Lo = g1_512_lo(W[i - 4], W[i - 3]);
      const c1Hi = W[i - 14];
      const c1Lo = W[i - 13];
      const c2Hi = g0_512_hi(W[i - 30], W[i - 29]);
...
function s0_512_hi(xh, xl) {
  ...
  const c1_hi = rotr64HI(xl, xh, 2);
  const c2_hi = rotr64HI(xl, xh, 7);
...
function s0_512_lo(xh, xl) {
  ...
  const c1_lo = rotr64LO(xl, xh, 2);
  const c2_lo = rotr64LO(xl, xh, 7);
...
var _Point = class _Point extends BasePoint {
  /**
   * @constructor
  ...
  static fromString(str) {
    ...
    return _Point.fromDER(bytes);
  }
  static redSqrtOptimized(y2) {
    const red = _Point.red;
    const p = red.m;
    const exponent = p.addn(1).iushrn(2);
  ...
  static fromX(x, odd) {
    if (typeof BigInt === "function") {
      let mod = function(a, n) {
        ...
        return result;
      }, sqrtMod = function(a, p2) {
        const exponent = p2 + BigInt(1) >> BigInt(2);
        const sqrtCandidate = modPow(a, exponent, p2);
        if (mod(sqrtCandidate * sqrtCandidate, p2) === mod(a, p2)) {
  ...
  add(p) {
    ...
    if (this.neg().eq(p)) {
      return new _Point(new BigNumber(0), new BigNumber(0));
    }
    if (((_a = this.x) == null ? void 0 : _a.cmp(p.x ?? new BigNumber(0))) === 0) {
      return new _Point(new BigNumber(0), new BigNumber(0));
    }
    let c = ((_b = this.y) == null ? void 0 : _b.redSub(p.y ?? new BigNumber(0))) ?? new BigNumber(0);
    if (c.cmpn(0) !== 0) {
      c = c.redMul(((_c = this.x) == null ? void 0 : _c.redSub(p.x ?? new BigNumber(0)).redInvm()) ?? new BigNumber(1));
    }
    const nx = c == null ? void 0 : c.redSqr().redISub(this.x ?? new BigNumber(0)).redISub(p.x ?? new BigNumber(0));
    const ny = (c ?? new BigNumber(1)).redMul((this.x ?? new BigNumber(0)).redSub(nx ?? new BigNumber(0))).redISub(this.y ?? new BigNumber(0));
    return new _Point(nx ?? new BigNumber(0), ny ?? new BigNumber(0));
  }
  /**
  ...
  _getBeta() {
    ...
    if (pre != null) {
      ...
      const endoMul = (p) => {
        ...
        if (curve.endo === void 0 || curve.endo === null) {
  ...
  _endoWnafMulAdd(points, coeffs, jacobianResult) {
    ...
    for (i = 0; i < points.length; i++) {
      ...
      let p = points[i];
      let beta = p._getBeta() ?? new _Point(new BigNumber(0), new BigNumber(0));
...
__publicField(_Point, "red", new ReductionContext("k256"));
__publicField(_Point, "a", new BigNumber(0).toRed(_Point.red));
__publicField(_Point, "b", new BigNumber(7).toRed(_Point.red));
__publicField(_Point, "zero", new BigNumber(0).toRed(_Point.red));
var Point = _Point;
...
var sign = (msg, key, forceLowS = false, customK) => {
  if (typeof BigInt === "function") {
    let bigIntToBytes = function(value, length) {
      const hex = value.toString(16).padStart(length * 2, "0");
      const bytes2 = new Uint8Array(length);
      for (let i = 0; i < length; i++) {
        bytes2[i] = parseInt(hex.substr(i * 2, 2), 16);
      }
      return bytes2;
    }, truncateToN2 = function(k, n2, truncOnly = true) {
      const kBitLength = k.toString(2).length;
      const nBitLength = n2.toString(2).length;
    ...
    }, generateK = function(customK2) {
      if (typeof customK2 === "function") {
        const kbn = customK2(iter);
        const kstr = kbn.toString(16);
        return BigInt("0x" + kstr);
      } else if (customK2 != null && BigNumber.isBN(customK2)) {
        const kstr = customK2.toString(16);
        return BigInt("0x" + kstr);
      } else {
    ...
    while (!validSignature) {
      ...
      if (forceLowS && s2 > n / two) {
        s2 = n - s2;
      }
      const rbn = new BigNumber(r2.toString(16), 16);
      const sbn = new BigNumber(s2.toString(16), 16);
...
var verify = (msg, sig, key) => {
  if (typeof BigInt === "function") {
    ...
    const modInv = (a, m) => {
      let [oldr, r2] = [a, m];
      let [olds, s2] = [BigInt(1), BigInt(0)];
      while (r2 !== zero) {
        const q = oldr / r2;
        [oldr, r2] = [r2, oldr - q * r2];
    ...
    const pointAdd = (P, Q) => {
      ...
      const X3 = modSub(modSub(modMul(r2, r2, p), HHH, p), modMul(two, V, p), p);
      const Y3 = modSub(modMul(r2, modSub(V, X3, p), p), modMul(S1, HHH, p), p);
    ...
    const verifyECDSA = (hash2, publicKey2, signature2) => {
      const { r: r2, s: s2 } = signature2;
      const z = hash2;
      if (r2 <= zero || r2 >= n || s2 <= zero || s2 >= n) {
        return false;
      }
      const w = modInv(s2, n);
      if (w === zero) {
      ...
      const u2 = modMul(r2, w, n);
      const RG = scalarMultiply(u1, G);
      const RQ = scalarMultiply(u2, publicKey2);
      const R2 = pointAdd(RG, RQ);
      if (R2.Z === zero) {
        return false;
      }
      const ZInv = modInv(R2.Z, p);
      if (ZInv === zero) {
        return false;
      }
      const ZInv2 = modMul(ZInv, ZInv, p);
      const x1affine = modMul(R2.X, ZInv2, p);
...
var PrivateKey = class _PrivateKey extends BigNumber {
  ...
  toKeyShares(threshold, totalShares) {
    ...
    for (let i = 0; i < totalShares; i++) {
      const x = new BigNumber(_PrivateKey.fromRandom().toArray());
...
var WalletError_default = walletErrors;
...
var VerifiableCertificate = class extends Certificate {
...
var SUCCESS_STATUS_CODES = [200, 402];
  ...
  async send(message) {
    ...
    if (message.messageType !== "general") {
      ...
      if (!SUCCESS_STATUS_CODES.includes(response.status)) {
        ...
        if ((errorInfo == null ? void 0 : errorInfo.status) === "error" && typeof errorInfo.description === "string") {
